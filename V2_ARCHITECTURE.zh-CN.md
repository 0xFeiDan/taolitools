...
盘前、正常时段、盘后分别拥有独立的 Dynamic Midline、波动率、Z-score 和 Regime
状态，防止盘后预言机机制污染正常交易时段统计。

如果 Pair 已经存在，而新时段统计尚未完成预热，EXIT 可以使用最近一个已就绪的
中枢作为保守的风险降低基准。临时全国休市或交易所特殊规则无法通过固定日历预测，
仍需依赖行情过期和交易所故障保护。
